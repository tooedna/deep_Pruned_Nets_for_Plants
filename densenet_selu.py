...
def __transition_up_block(ip, nb_filters, type='deconv', weight_decay=1E-4):
    ...
    if type == 'upsampling':
        ...
        x = Conv2D(nb_filters, (3, 3), activation='selu', padding='same', kernel_regularizer=l2(weight_decay),
                   use_bias=False, kernel_initializer='he_normal')(ip)
        x = SubPixelUpscaling(scale_factor=2)(x)
        x = Conv2D(nb_filters, (3, 3), activation='selu', padding='same', kernel_regularizer=l2(weight_decay),
                   use_bias=False, kernel_initializer='he_normal')(x)
    else:
        x = Conv2DTranspose(nb_filters, (3, 3), activation='selu', padding='same', strides=(2, 2),
